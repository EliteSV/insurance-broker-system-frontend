...
function UsuariosPage() {
    ...
    const handleOk = () => {
        if (id) {
            eliminarUsuario(id).unwrap().then(() => {
    ...
    const columns: TableProps<Usuario>['columns'] = [
        ...
        },
        {
            ...
            render: (_, record) => (
                <Space size="middle">
                    <Link to={`/usuarios/${record.id}`}><EyeOutlined /></Link>
                    <Link to={`/usuarios/modificar/${record.id}`}><EditOutlined /></Link>
    ...
    return (
        <PageLayout>
            ...
            <Modal title="Eliminar usuario" open={isModalOpen} onOk={handleOk} onCancel={handleCancel} confirmLoading={eliminarUsuarioResult.isLoading} okText="Si" cancelText="No">
