...
const PagoForm = ({ initialValues, isLoading, onFinish, submitText = 'Registrar' }: Props) => {
    ...
    const polizaOptions = polizas?.map((poliza: any) => ({ label: poliza.codigo, value: poliza.id }));
    let vigenciaOptions = []
    if (polizaId) {
        vigenciaOptions = polizas?.find((poliza: any) => poliza.id === polizaId)?.vigencias.map((vigencia: any) => {
            const fecha_inicio = dayjs(vigencia.fecha_inicio).format('DD/MM/YYYY');
            const fecha_vencimiento = dayjs(vigencia.fecha_vencimiento).format('DD/MM/YYYY');
            return { label: `${fecha_inicio} - ${fecha_vencimiento}`, value: vigencia.id }
        });
    }
    ...
    return (
        ...
        >
            <Form.Item
                label="N° de Cuota"
                name="cuota"
                rules={[{ required: true, message: 'Por favor, ingrese el numero de cuota!' }]}
            >
            ...
            {estado === EstadoPago.Pagado && (
                <>
                    ...
                    <Form.Item
                        name="comprobante"
                        label="Comprobante"
                        rules={[{ required: true, message: 'Por favor, selecciona una imagen del comprobante.' }]}
                    >
            ...
            <Form.Item
                ...
                rules={[{ required: true, message: 'Por favor, seleccione la vigencia' }]}
            >
                <Select placeholder="Seleccione la vigencia" options={vigenciaOptions} />
            </Form.Item>
